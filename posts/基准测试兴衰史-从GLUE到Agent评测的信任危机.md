...
每次新模型发布，技术报告里的 benchmark 表格都更长、分数都更高，但"哪个模型更好用"这个问题反而越来越难回答。上一篇讲 vLLM 和 SGLang 时我领教过推理引擎 benchmark 的罗生门，而模型侧的 benchmark 圈比那边还要乱一个量级——同一份数据集上，各家报的分数可以差出 5 个点，原因藏在 few-shot 数量、答案提取方式、温度参数这些从来不写进新闻稿的细节里。
